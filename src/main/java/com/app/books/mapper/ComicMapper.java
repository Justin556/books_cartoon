...
public interface ComicMapper {
    ...
    @Select("<script>SELECT COUNT(zong.coverPic) as sum,zong.coverPic,zong.comicId,category,title,status from (select `status`,title,category,cover_pic as coverPic,detail_pic as detailPic,tcl.comic_id as comicId from t_comic tc\n" +
            "left JOIN t_comic_likes tcl on tcl.comic_id=tc.id)as zong\n" +
            "group by zong.coverPic,zong.comicId,category,title,status\n" +
            "order by sum desc </script>")
    List<Comic> ranking(ComicQuery comicQuery);
