...
public interface PromoteMapper {
    ...
    @Select("<script>  select (SELECT tu.user_name from t_user as tu where id=trl.user_id)  as aliName,trl.user_id as id from t_user_retail_level as trl where parent_id=#{id} </script>")
